...
def copy_examples_with_bundle_url(examples_dir: Path, bundle_url: str) -> list[Path]:
    ...
    for example in examples:
        ...
        if count != 1:
            raise SystemExit(f"{example.name} does not declare the expected rand package dependency")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roc-random-bundle-", dir=tmp_parent) as tmp:
        tmp_dir = Path(tmp)
